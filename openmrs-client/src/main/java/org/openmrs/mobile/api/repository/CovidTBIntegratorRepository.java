...
public class CovidTBIntegratorRepository extends RetrofitRepository {

    private OpenMRSLogger logger;
    private RestApi restApi;

    public CovidTBIntegratorRepository(){
    ...
    public void syncCovidTBIntegrator(final Receipt receipt, @Nullable final DefaultResponseCallbackListener callbackListener) {
        if (NetworkUtils.isOnline()) {
                ...
                public void onResponse(@NonNull Call<Receipt> call, @NonNull Response<Receipt> response) {
                    try{
                        ...
                        if (response.isSuccessful()) {
                            OpenMRSCustomHandler.writeLogToFile("Covid TB Sync is successful");
                            ...
                            Log.v("Baronearl", "Failed Receipt: " + response.code() + " / " + response.message() + " / " + response.body() + " / " + response.errorBody());
                        }
                    }catch (Exception e){
                        Log.v("Baron Error", e.getMessage());
                    }
                }

                @Override
                public void onFailure(@NonNull Call<Receipt> call, @NonNull Throwable t) {
                    OpenMRSCustomHandler.writeLogToFile("Covid19 TB Integrator The response code is = " + t.getMessage());
                    if (callbackListener != null) {
            ...
            OpenMRSCustomHandler.writeLogToFile("Network is offline to Sync Covid19TB Integrator");
            if (callbackListener != null) {
                callbackListener.onResponse();
            }
        }
